...
class Preferences(BaseModel):
    ...
    crossings: int = Field(0, ge=0, le=100, description="0=don't care, 100=avoid road crossings")
    tradeoff: int = Field(50, ge=0, le=100, description="0=distance accuracy only, 100=route quality only")
...
class GenerateRequest(BaseModel):
    ...
    mode: str = Field("walk", pattern="^(walk|cycle)$")
    preferences: Preferences = Field(default_factory=Preferences)
    algorithm: int = Field(1, ge=1, le=3, description="1=isochrone-polygon")
